...
export const POST = async (request: NextRequest) => {
  ...
  await dbConnect();
  if (type === "verify user") {
    const data = await optVerificationService.verifyUserService(email, currentTime);
...
export const PATCH = async (request: NextRequest) => {
  ...
  if (type === "reset-password") {
    ...
    await optVerificationService.updatePasswordService(email, currentPassword, password);
  }
};
